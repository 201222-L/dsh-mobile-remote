...
export function verifyUpdateAuth({ headers, method, path, query = {}, bodyShaHex = "", token, nonceCache }) {
  ...
  // P0 修复（full review）：**先验 HMAC，再登记 nonce**——若先登记，攻击者可用大量错误签名
  // 的 nonce 填满缓存（4096 容量）挤掉已验证请求的 nonce，形成 60 秒内重放（Codex 已复现）。
  // 顺序：时间窗 → nonce 长度 → HMAC 验签 → 登记（非自身重放由缓存的 old entry 拒绝）。
  const nonceBytes = b64uDecode(nonce);
