...
import { createReadStream, readFileSync, writeFileSync, mkdirSync, existsSync, copyFileSync } from "node:fs";
...
export async function stagePluginUpdate({ manifest, source, declaredAppVersionCode, state, fetchImpl = fetch, logger, trustedKeys = trustedReleaseKeys }) {
  ...
  // 取包：github → 直连 Release 资产；local-cache → 更新目录
  let srcPath = null;
  if (source === "github") {
    ...
    const info = { size: buf.length, sha256: createHash("sha256").update(buf).digest("hex") };
    if (info.sha256 !== artifact.sha256 || info.size !== artifact.sizeBytes) throw new Error("插件产物校验失败");
    mkdirSync(join(stageDir), { recursive: true });
    writeFileSync(join(stageDir, artifact.fileName), buf);
...
/** /update-file 响应（流式转发）；返回 { error }；artifactId 白名单来自当前已验签 manifest。 */
