...
export function apply(ctx, config) {
	...
	const notifyNeedsAnswer = (sessionId, detail) => {
		...
		pushSend("needs-answer", sessionId, sessionTitleOf(sess) ?? sessionId.slice(0, 8) + "…", detail).catch(() => {}); // review：浮动 promise 兜底
		return true;
	};
	...
	const armDone = (sessionId, kind, detail) => {
		...
		const handle = setTimeout(async () => {
			try {
				...
				pushSend(pending.kind, sessionId, sessionTitleOf(session) ?? sessionId.slice(0, 8) + "…", pending.detail).catch(() => {}); // review：浮动 promise 兜底
			} catch (err) {
	...
	const onAgentStatus = (payload) => {
		...
		const sessionId = agentSession?.id ?? String(payload.agent?.id ?? "").replace(/^session:/, "");
		broadcast({
			type: "agent/status",
			sessionId,
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/messages") {
			...
			try {
				...
				// review：超时/中止映射 504，其余按内核状态（502 为 RPC 传输层失败）
				const status = err?.status === 502
					? 502
					: err?.name === "TimeoutError" || err?.name === "AbortError"
						? 504
						: 400;
				return error(res, status, err?.code ?? "update-queue-failed", err?.message ?? String(err));
			}
			return;
		}
		...
		if (rest === "/catalog") {
			...
			let firstCfg;
			try {
				firstCfg = firstAgent ? await readSessionConfig(firstAgent.id) : undefined;
			} catch {
				firstCfg = undefined;
			}
			const defaults = {
	...
	ctx.effect(() => {
		...
		const unsubscribeDisposed = ctx.on("agent/disposed", ({ agent }) => {
			const sid = agent?.session?.id ?? String(agent?.id ?? "").replace(/^session:/, "");
			if (!sid) return;
			cancelDone(sid);
