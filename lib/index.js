...
export function apply(ctx, config) {
	...
	const pushNotification = (sessionId, kind, detail) => {
		...
		const now = Date.now();
		// 聚合：同一会话同一类型的通知合并（刷新标题/详情/时间，保留原 id 与已读状态），
		// 避免多轮任务按轮次刷满通知列表。
		let existing = null;
		for (const [id, n] of notifStore) {
			if (n.sessionId === sessionId && n.kind === kind) {
				existing = { id, n };
				break;
			}
		}
		if (existing) {
			notifStore.set(existing.id, { ...existing.n, title, detail, time: now });
		} else {
	...
	const pushSend = async (kind, sessionId, title, detail) => {
		...
		for (const target of config.pushUrls) {
			await pushToChannel(target, kindLabel, title, detail, sessionId).catch((err) => {
				ctx.logger.warn(`mobile-remote: push to "${target.name}" failed: ${err?.message ?? err}`);
			});
		}
	};
	const pushToChannel = async (target, kindLabel, title, detail, sessionId) => {
		...
		if (target.format === "serverchan") {
			...
			const params = new URLSearchParams({ title: `${kindLabel} · ${title}`, desp: detail });
			...
			init = { method: "POST", headers: { ...headers, "content-type": "application/json" }, body: JSON.stringify({ title: `${kindLabel} · ${title}`, message: detail }) };
		} else if (target.format === "bark") {
			init = { method: "POST", headers: { ...headers, "content-type": "application/json" }, body: JSON.stringify({ title: `${kindLabel} · ${title}`, body: detail }) };
		} else {
			init = { method: "POST", headers: { ...headers, "content-type": "application/json" }, body: JSON.stringify({ kind, title, detail, sessionId, time: Date.now() }) };
		}
	 ...
	 *  避免把不可达地址（如 198.18.0.1）当成首选扫码地址。 */
	const ipv4Addresses = () =>
	...
	const trustedHosts = () => new Set(["127.0.0.1", "localhost", ...ipv4Addresses()]);
	...
	const readSessionConfig = async (sessionId) => {
		const config = { model: undefined, reasoningEffort: undefined, permissionPreset: undefined, agentPreset: undefined };
		const sessions = ctx.get("sessions");
		const session = sessions?.get(sessionId);
		try {
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest !== "/qr-config" && !authorized(req)) {
			error(res, 401, "auth-required", authEnabled ? "访问口令未通过验证" : "认证未启用");
			return;
		}
		...
		if (rest === "/bootstrap") {
			...
			const sessionList = sessions
				...
				: [];
			const urls = [...ipv4Addresses(), "127.0.0.1"].map((ip) => `http://${ip}:${ctx.webServer.port}`);
			sendJson(res, 200, {
				ok: true,
		...
		if (rest === "/qr-config") {
			...
			sendJson(res, 200, {
				ok: true,
				urls: [...ipv4Addresses(), "127.0.0.1"].map((ip) => `http://${ip}:${ctx.webServer.port}${basePath}`),
		...
		if (rest === "/catalog") {
			...
			try {
				if (firstAgent) {
					...
					if (llm) pushModels("deepseek-official", await llm.listModels("deepseek-official"));
				}
			} catch {
		...
		if (rest === "/balance") {
			...
			try {
				...
				if (!response.ok) return error(res, 502, "balance-failed", `DeepSeek API HTTP ${response.status}`);
				const data = await response.json();
				sendJson(res, 200, { ok: true, balance: data });
			} catch (err) {
		...
		if (rest === "/usage") {
			...
			sendJson(res, 200, { ok: true, sessionId, usage: total });
			return;
		}
		...
		if (rest === "/workspaces") {
			...
			sendJson(res, 200, {
				ok: true,
				workspaces: workspaces.map((w) => ({ id: w.id, path: w.path, title: w.title })),
			});
			return;
		}
