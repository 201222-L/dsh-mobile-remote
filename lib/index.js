...
export function apply(ctx, config) {
	...
	const pushNotification = (sessionId, kind, detail) => {
		...
		broadcast({ type: "mobile/notify", notification: { id, kind, sessionId, title, detail, time } });
	};
	...
	const armDone = (sessionId, kind, detail) => {
		...
		const handle = setTimeout(async () => {
			...
			pushSend(pending.kind, sessionId, sessionTitleOf(session) ?? sessionId.slice(0, 8) + "…", pending.detail);
		}, config.doneGraceMs);
		handle.unref?.();
