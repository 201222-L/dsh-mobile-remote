...
export function apply(ctx, config) {
	...
	// 错误码兜底 fallbackCode。展开进 error()：error(res, ...rpcError(err, "xxx-failed"))
	const rpcError = (err, fallbackCode) => {
		const status = err?.status === 502
			...
			: err?.name === "TimeoutError" || err?.name === "AbortError"
				? 504
				: err?.status ?? 400;
		return { status, code: err?.code ?? fallbackCode, message: err?.message ?? String(err) };
	};
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/commands") {
			...
			if (method === "GET" || method === "HEAD") {
				...
				if (!sessionId) return error(res, 400, "bad-request", "missing sessionId");
				const agent = agents ? agents.get(sessionId) : undefined;
				if (!commands || !agent) return error(res, 503, "commands-unavailable");
				try {
