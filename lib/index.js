...
import { verifyUpdateRequest, updateEndpointPath } from "./update-endpoints.js";
import { scanAndVerifyManifest, serveArtifactFile, stagePluginUpdate, UpdateState } from "./update-endpoints.js";
...
export function apply(ctx, config) {
	...
	// ── M2 电脑源/插件联动：更新端点共享状态（updateToken 认证 + 已验签 manifest 上下文） ──
	const updateState = new UpdateState();
	const EMPTY_BODY_SHA = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"; // sha256("")
	const handleUpdateEndpoint = async (updatePath, fullPath, req, res, method) => {
		// canonical 的 path = 请求完整 pathname（含挂载前缀）——App 端对同一字符串签名
		const authHeadersCheck = (_pathIgnored, bodyShaHex) => verifyUpdateRequest({
			headers: req.headers,
			method: method === "POST" ? "POST" : "GET",
			path: fullPath,
			query: {},
			bodyShaHex,
			nonceCache: updateState.nonceCache,
		});
		if (updatePath === "/plugin-update") {
			if (method !== "POST") return error(res, 405, "method-not-allowed");
			const raw = await readBody(req, 512 * 1024);
			const bodyShaHex = createHash("sha256").update(raw).digest("hex");
			const auth = authHeadersCheck("/plugin-update", bodyShaHex);
			if (!auth.ok) return error(res, 401, "update-auth-required", auth.reason);
			let doc;
			try { doc = JSON.parse(raw); } catch { return error(res, 400, "bad-json"); }
			try {
				const r = await stagePluginUpdate({
					manifest: doc.manifest,
					source: doc.source,
					declaredAppVersionCode: doc.declaredAppVersionCode,
					state: updateState,
					logger: ctx.logger,
				});
				...
				return error(res, status, "plugin-update-failed", e?.message ?? String(e));
			}
		}
		if (method !== "GET") return error(res, 405, "method-not-allowed");
		const auth = authHeadersCheck(updatePath, EMPTY_BODY_SHA);
		if (!auth.ok) return error(res, 401, "update-auth-required", auth.reason);
		if (updatePath === "/update-check") {
			try {
				const found = scanAndVerifyManifest(updateState.updatesDir);
				if (!found) return error(res, 404, "no-manifest", "更新目录无已验签 manifest");
				updateState.rememberVerified(found.manifest, found.digest);
				return sendJson(res, 200, { ok: true, manifest: found.manifest });
			} catch (e) {
				return error(res, 500, "update-check-failed", e?.message ?? String(e));
			}
		}
		const m = /^\/update-file\/([^/]+)$/.exec(updatePath);
		if (!m) return error(res, 404, "not-found");
		try {
			const r = await serveArtifactFile(m[1], updateState, res);
			if (r.error) return error(res, 404, r.error);
		} catch (e) {
			return error(res, 500, "update-file-failed", e?.message ?? String(e));
		}
	};
	...
	const handleApi = async (req, res, url, rest) => {
		...
		// ── v3.1.0 M2：更新端点先于全局 authToken 分流，由 updateToken/HMAC 单独认证 ──
		const updatePath = updateEndpointPath(rest);
		if (updatePath) {
			await handleUpdateEndpoint(updatePath, url.pathname, req, res, method);
			return;
		}
		...
		if (rest === "/bootstrap") {
			...
			sendJson(res, 200, {
				...
				// v3.1.0 M2：能力字段（老 App 以 capability 判断而非版本号猜测）
				capabilities: ["plugin-update-endpoint"],
				// M2 配对通道：updateToken 随 bootstrap 下发（qr-config 字段供桌面 GUI 用；
				// QR 文本格式由桌面端生成，本插件不直接生成——bootstrap 对持有 authToken 的手机可达）
				updateToken: updateState.token(),
		...
		if (rest === "/qr-config") {
			...
			sendJson(res, 200, {
				...
				// v3.1.0 M2：更新通道凭据（与 authToken 隔离；随扫码/配对下发，轮换后须重新扫码）
				updateToken: updateState.token(),
				path: basePath,
			});
			return;
