	...
	const finishApproval = async (entry, outcome) => {
		...
		try {
			...
			if (result === false) {
				...
				if (entry.timedOut) {
					// The Gateway delivery needed to settle this event is gone (stream
					// disconnected mid-flight), so no receipt can ever succeed. Leaving
					// the entry pending would hold the Host waterfall forever: force the
					// close and let the Gateway fail the still-pending delivery closed.
					ctx.logger.warn?.(`mobile-remote: approval/request 结算回执丢失（${entry.phoneId}）→ 强制收卡（Host 侧按 fail-close 处理）`);
					rememberSettledEvent(entry.rpcId);
	...
	const finishQuestion = async (entry, value, { reject = false } = {}) => {
		...
		try {
			...
			if (result === false) {
				entry.settling = false;
				if (entry.timedOut) {
					ctx.logger.warn?.(`mobile-remote: user-questions/request 结算回执丢失（${entry.phoneId}）→ 强制收卡（Host 侧按 fail-close 处理）`);
					rememberSettledEvent(entry.rpcId);
