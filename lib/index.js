...
export function apply(ctx, config) {
	...
	const handleUpdateEndpoint = async (updatePath, req, res, method) => {
		const authHeadersCheck = (path, bodyShaHex) => verifyUpdateRequest({
			headers: req.headers,
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (updatePath) {
			await handleUpdateEndpoint(updatePath, req, res, method);
			return;
		}
