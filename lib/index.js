...
export function apply(ctx, config) {
	...
	const onSessionEvent = (session, event) => {
		...
		//   子代理会话（header.parentSession 有值）是父任务的一部分，一律不通知完成/失败
		if (event.type === "turn/end") {
			...
			if (notifKind) {
				...
				} else if (session.header?.parentSession) {
					// 子代理会话：不通知（它是父任务的一部分，父任务结束才通知）
					cancelDone(session.id);
				} else {
	...
	const onAgentStatus = (payload) => {
		...
		broadcast({
			...
			child: Boolean(agentSession?.header?.parentSession),
		});
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/bootstrap") {
			...
			const sessionList = sessions
				...
				: [];
			// v2.9.0：LAN 桥启用时首选地址 = 桥地址（手机可达），回环地址仅本机自连兜底
			const urls = lanEnabled
		...
		if (rest === "/qr-config") {
			...
			// v2.9.0：LAN 桥启用 → 二维码首要地址 = 桥地址（手机扫码直连；本机回环地址对手机不可达，不再展示）
			const qrUrls = lanEnabled
		...
		if (rest === "/queue") {
			...
			if (!agent) {
				// review：会话存在但无 agent（休眠）→ 空队列（与 PC 空队列一致）；真不存在才 404
				if (!sessions?.get(sessionId)) return error(res, 404, "session-not-found");
				sendJson(res, 200, { ok: true, queue: [] });
				return;
			}
			const inbox = agent.inbox;
			const rows = [];
			// review：透传 placement 语义——next-turn 为可插话的 "queued"，
			// next-step 为 "steering"（steer 仅对 queued 有效，客户端据此禁用插话按钮）
			const pushRows = (list, placement) => {
				for (const msg of list ?? []) {
					const text = messageTextOf(msg);
					// review：非文本/空文本消息也保留行（text 为 null，客户端显示占位并可删除）
					rows.push({ id: msg.id, text: text || null, placement });
				}
			};
			pushRows(inbox?.nextTurn, "queued");
			// review(M2)：对齐内核 queueItems 语义——next-step 区分 user 来源(steering)
			// 与非 user 来源(context 档)，避免 REST 快照与 SSE session/queue 帧语义冲突
			pushRows(inbox?.nextStep?.filter((m) => m?.source?.kind !== "user"), "context");
			pushRows(inbox?.nextStep?.filter((m) => m?.source?.kind === "user"), "steering");
			sendJson(res, 200, { ok: true, queue: rows });
			return;
		}
		...
		if (rest === "/diagnostics") {
			...
			sendJson(res, 200, {
				...
				runtime: {
					...
					cwd: process.cwd(),
					authEnabled,
					// v2.9.0：LAN 桥状态（enabled/配置端口 vs listening=实际监听成功）
					lanBridge: { enabled: lanEnabled, host: lanHost, port: lanPort, listening: Boolean(lanServer?.listening) },
				},
				services,
				checks,
			});
			...
			if (!authEnabled) {
				...
				try {
					lanServer = createHttpServer((req, res) => {
						...
						const mobile = pathname === basePath || pathname.startsWith(`${basePath}/`);
						if (!mobile || pathname === `${basePath}/api/qr-config` || pathname === `${basePath}/qr.png`) {
							res.writeHead(404);
							res.end();
							return;
						}
					...
					lanServer.on("error", (err) => ctx.logger.warn(`mobile-remote: lanBridge 监听错误：${err?.message ?? err}`));
					lanServer.listen(lanPort, lanHost, () => {
						ctx.logger.info?.(`mobile-remote: lanBridge 已监听 ${lanHost}:${lanPort} → 127.0.0.1:${ctx.webServer.port}${basePath}`);
					});
				} catch (err) {
