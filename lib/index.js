...
export function apply(ctx, config) {
	...
	/** 用户消息 content blocks → 纯文本（队列视图显示用）。 */
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/queue") {
			...
			const pushRows = (list) => {
				for (const msg of list ?? []) {
					...
					if (text) rows.push({ id: msg.id, text, placement: "queued" });
				}
			};
			pushRows(inbox?.nextTurn);
			pushRows(inbox?.nextStep);
		...
		if (rest === "/messages" && method === "POST") {
			...
			if (action.kind === "edit" && !Array.isArray(action.content)) {
				return error(res, 400, "bad-request", "edit requires content blocks");
			}
			try {
				...
				return error(res, err?.status === 502 ? 502 : 400, err?.code ?? "update-queue-failed", err?.message ?? String(err));
			}
			return;
		}
