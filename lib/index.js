...
export function apply(ctx, config) {
	...
	const pushNotification = (sessionId, kind, detail) => {
		...
		if (existing) {
			...
			// 聚合 = 新活动：若该通知此前已读，重新标记未读（否则同会话后续完成
			// 永远合并进旧已读通知，App 角标永不亮）
			if (readIds.delete(existing.id)) persistReadIds();
		} else {
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/sessions") {
			...
			if (records) {
				...
				// 批量 readTitleSnapshots 一次调用取全部标题，避免逐个读日志拖慢列表（归档后刷新几秒才生效）
				const dormant = records.filter((r) => !sessions.get(r.header.id));
				const titleMap = new Map();
				if (dormant.length > 0 && query?.readTitleSnapshots) {
					try {
						const snaps = await query.readTitleSnapshots(dormant.map((r) => r.header.id));
						dormant.forEach((r, i) => {
							const s = snaps?.[i];
							if (s?.status === 'fulfilled') {
								const t = s.value?.title?.title;
								if (t) titleMap.set(r.header.id, t);
							}
						});
					} catch {
						// 批量失败则逐个兜底
				...
				} else if (dormant.length > 0 && query?.readTitleSnapshot) {
					for (const r of dormant) {
						try {
							const snap = await query.readTitleSnapshot(r.header.id);
							const t = snap?.title?.title;
