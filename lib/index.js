...
export function apply(ctx, config) {
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/respond") {
			...
			if (body?.kind === "question") {
				const qEntry = pendingQuestions.get(String(body?.questionId ?? ""));
				if (qEntry && Array.isArray(body?.answers)) {
					qEntry.resolve({ answers: body.answers });
					sendJson(res, 200, { ok: true, accepted: true });
					return;
				}
				...
				const aEntry = pendingApprovals.get(String(body?.approvalId ?? ""));
				if (aEntry) {
					// outcome: "allowed-once" | "rejected" | 其他 → fail-close（unavailable）
					aEntry.resolve(outcome === "allowed-once" || outcome === "rejected" ? outcome : "unavailable");
					sendJson(res, 200, { ok: true, accepted: true });
					return;
				}
			}
	...
	// 手机 App 在线（存在 SSE 连接）时接管：转发 mobile/frame + /respond 结算；
	// 离线或未应答则超时 fail-close（与内核默认一致）；拒接时 next() 交其他 answerer（桌面 GUI）。
	const pendingApprovals = new Map(); // approvalId -> { sessionId, resolve }
	const pendingQuestions = new Map(); // questionId -> { sessionId, resolve }
	ctx.effect(() => {
		...
		// 手机在线即接管应答（手机弹卡），离线/拒接 next() 落回桌面 GUI answerer。
		const offApproval = ctx.on("approval/request", (req, next) => {
			if (connections.size === 0) {
				ctx.logger.info?.("mobile-remote: approval/request 手机离线 → 交给其他 answerer（桌面 GUI）");
				return next();
			}
			const sessionId = req?.agent?.session?.id ?? "";
			if (!sessionId) return next();
			const approvalId = typeof req?.callId === "string" && req.callId !== ""
			...
			return new Promise((resolve) => {
				const entry = {
					...
					resolve: (outcome) => {
						...
						ctx.logger.info?.(`mobile-remote: approval/request 结算（${approvalId} → ${outcome}）`);
						resolve(outcome);
					},
				};
				const timer = setTimeout(() => {
					// 手机无响应：fail-close（与内核 unavailable 语义一致），不悬挂工具调用
					if (pendingApprovals.get(approvalId) === entry) {
						pendingApprovals.delete(approvalId);
						ctx.logger.warn?.(`mobile-remote: approval/request 手机超时未应答 → unavailable（${approvalId}）`);
						resolve("unavailable");
					}
				}, 120_000);
				pendingApprovals.set(approvalId, entry);
				broadcast({ type: "mobile/frame", frame: { type: "approval/requested", approvalId, sessionId, toolName: req?.toolName, reason: req?.reason } });
		...
		const offQuestion = ctx.on("user-questions/request", (req, next) => {
			if (connections.size === 0) return next();
			const sessionId = req?.agent?.session?.id ?? "";
			if (!sessionId) return next();
			const questionId = `${sessionId}:${Date.now().toString(36)}`;
			...
			return new Promise((resolve) => {
				const entry = {
					...
					resolve: (value) => {
						clearTimeout(timer);
						if (pendingQuestions.get(questionId) === entry) pendingQuestions.delete(questionId);
						resolve(value);
					},
				};
				const timer = setTimeout(() => {
					if (pendingQuestions.get(questionId) === entry) {
						pendingQuestions.delete(questionId);
						resolve(null);
					}
				}, 120_000);
				pendingQuestions.set(questionId, entry);
				broadcast({ type: "mobile/frame", frame: { type: "question/requested", questionId, sessionId, questions: req?.request?.questions ?? [] } });
