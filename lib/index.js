...
export function apply(ctx, config) {
	...
	const heldQueue = new Map(); // sessionId -> [{ id, text, at }]
				...
				for (const [sid, list] of Object.entries(held)) {
					if (Array.isArray(list)) {
						heldQueue.set(sid, list
							.filter((m) => typeof m?.id === "string" && m.id !== "" && typeof m?.text === "string")
							.map((m) => ({ id: m.id, text: m.text, at: Number(m.at) || 0 })));
					}
				}
			}
		} catch {
	...
	const queueRowsOf = (sessionId) => {
		...
		for (const held of heldOf(sessionId)) rows.push({ id: held.id, text: held.text, placement: "queued" });
		return rows;
	};
	...
	/** agent 空闲 → 释放全部持存消息（按发出时序 followup，新轮次执行）。 */
		...
		if (rest === "/send") {
			...
			try {
				if (steer && target.status !== "idle" && typeof target.steer === "function") {
					...
					heldQueue.set(target.id, [...heldOf(target.id), { id: message.id, text, at: Date.now() }]);
					persistHeld();
