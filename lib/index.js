...
export function apply(ctx, config) {
	...
	const onSessionEvent = (session, event) => {
		...
		//   是用户自己的平行对话，照常通知**（v3.0.1 修正：此前按 parentSession 判定把 fork 也抑制了）
		if (event.type === "turn/end") {
			...
			if (notifKind) {
				if (notifKind === "needs-answer") {
					...
					// fork 会话无 origin 标记 → 走正常通知（v3.0.1 修正）
					cancelDone(session.id);
				} else {
	...
	const onAgentStatus = (payload) => {
		...
		broadcast({
			...
			// v3.0.1：child 判定对齐内核——仅 origin === "subagent" 是真子代理
					...
					try {
						for await (const envelope of proxy.events.mux({}, sig.signal)) {
							...
							if (frame.type === "question/requested" || frame.type === "approval/requested") {
								...
								// v3.0.0(队列同步修复):内核每次 inbox 变化都会广播 session/queue 快照
								// (认领/删除/编辑即时反映)。此前 App 端丢弃该帧、靠 400ms 节流 REST + 20s
								// 轮询刷新,存在陈旧窗口:排队消息被 agent 认领后 dock 仍显示、删除必然失败。
								// 现在归一化为 mobile/queue 帧(与 GET /queue 同款 rows 形状),App 端即时镜像。
								const sid = typeof frame.sessionId === "string" ? frame.sessionId : "";
								const rows = (Array.isArray(frame.items) ? frame.items : [])
									.map((it) => ({
										id: String(it?.id ?? ""),
										text: messageTextOf(it?.message),
										placement: it?.placement,
									}))
									.filter((row) => row.id !== "");
								broadcast({ type: "mobile/queue", sessionId: sid, rows });
							}
			...
			if (!authEnabled) {
				...
				try {
					lanServer = createHttpServer((req, res) => {
						...
						upstream.setTimeout(isSse ? 60_000 : 15_000, () => upstream.destroy());
						guardRes(res);
					...
					lanServer.requestTimeout = 60_000;
