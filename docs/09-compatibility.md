...
| dsh-mobile-remote 插件 | **v2.6.0（与 App/git tag 版本号统一）**；`/m/api/diagnostics` 可自检 |
| 手机 App（Android） | v2.6.0（与插件同版本 = 完美配对；不同版本可用但"谁旧谁吃亏"，详见 README「版本与兼容」）；Android 7.0+、64 位机型 |
