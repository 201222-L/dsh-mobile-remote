...
### 5B-2. 托管式穿透（SakuraFrp/樱花映射 等，免自建 VPS）

托管服务替你把 frps 跑在公网节点上，家里电脑只跑他们的启动器（frpc），步骤与 5B 相同但更省事：

1. 注册并实名（国内法规要求）→ 创建 **TCP 隧道**（本地 `127.0.0.1:3080`，得到 `节点域名:端口`）或 **HTTP(S) 隧道**（得到 `xxx.natfrp.net` 子域名，自带证书，推荐）。
2. 家里电脑安装并保持运行其启动器。
3. 插件配置同上：`trustedHosts: ["<节点域名或子域名>"]` + 强口令。
4. 手机 App 手动输入 `http://<节点域名>:<端口>`（或 `https://xxx.natfrp.net`）+ 口令。
5. **验证**：手机关 WiFi 用流量连接，状态点变绿并能收发消息即成功。
> 免费版有限速/流量限制（日常聊天够用）；换节点/重建隧道后地址会变，需同步更新手机地址与 `trustedHosts`。
