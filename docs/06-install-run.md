...
1. **电脑**：安装[蒲公英客户端](https://pgy.oray.com/)，登录账号，创建/加入一个**智能组网**（免费版支持 3 个成员）。加入后电脑会获得一个虚拟 IP（客户端界面可见，形如 `172.16.x.x`）。
2. **手机**：安装蒲公英 App，登录**同一账号**，加入**同一组网**，确认手机与电脑都显示「在线」。
...
- **虚拟组网类**（Tailscale / ZeroTier / 路由器 WireGuard）：电脑加入组网后其虚拟 IP 自动进入地址列表，无需改插件配置；手机首连手动填一次虚拟 IP 即可（Tailscale 国内控制面不可达，不推荐）。
...
- insert:
    - id: mobile-remote
      ...
      config:
        ...
        trustedHosts: ["<中继域名或IP>"]    # 如 frp-boy.com / xxx.natfrp.net
```
