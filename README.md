...
- 📱 **手机远程操作**：发消息/派任务、流式回复、Markdown 渲染、token 用量、上翻历史
- 🔔 **系统通知**：agent 完成 / 需要你回答 / 失败 → 手机弹窗（Server酱 / ntfy / Bark 任选；同会话同类型自动聚合）
- 💬 **会话管理**：查看/切换/续接全部会话，标题与 PC 端实时同步，按工作区分组（子目录自动归属最近工作区）
...
1. 与电脑同一 WiFi（或 Tailscale 组网，见 docs/06 §5）
...
- **dsh 版本**：锁定 `0.1.0-rc.6` 验证；升级策略见 docs/00-开发总纲.md
- **平台**：Windows/macOS + Android/iOS（跨平台组合全部支持）
- **个性化**：模型/权限/Agent 预设动态读取 PC 端真实目录，用户自定义自动出现
- **深度魔改 web profile**（禁用标准服务）：对应功能自动隐藏，不崩溃

## 文档

- `README.md` — 项目总览：功能、安装、配置、注意事项
- `FAQ.md` — 常见问题速查（连接/使用/安全）
- `CHANGELOG.md` — 版本历史
