...
> **推送与公网访问配置说明（重要）**：本项目的推送消息通道（Server酱 / ntfy / Bark / 自建等）与**非局域网使用方式**（蒲公英 / Tailscale / FRP 内网穿透等）**均为按需自配置，没有统一标准**——`cordis.patch.yml` 里的 `pushUrls`（推送通道）、`trustedHosts`（公网中继域名）、`lanBridge`（桌面版 LAN 桥）等字段请**按自己的需求修改**，文档（docs/06、docs/04-security）只给示例与安全边界，不强制任何固定方案。
