...
- **双端竞态与断流**：桌面先答的 cancel 帧与手机 `/respond` 并发时按已结算幂等返回（不再误报 `503`）；`$events` 流断开时清理本客户端专属的待办条目与手机卡片（Gateway 不会给已断开的 client 补发 cancel，否则卡片会残留到 120s）；结算回执丢失（断流）时强制收卡并让 Host 按 fail-close 结束，不把瀑布挂死；定时器重臂修复（结算失败不再留下已触发的一次性 timer）；`/respond` 取消必须携带 `sessionId`。
